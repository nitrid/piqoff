        ...
        let posQuery = 
        {
            query : `SELECT
                    ...
                    WHERE
                        ...
                        AND (AUDIT_LOG.DOC IS NULL OR AUDIT_LOG.STATUS = 0) `
        }
    ...
    async customerProcess()
    {
        ...
        let customerQuery = 
        {
            query : `WITH FumaGrouped AS (
                    ...
                    WHERE
                       (AUDIT_LOG.DOC IS NULL OR AUDIT_LOG.STATUS = 0);`,
        }
    ...
    async pointProcess()
    {
        ...
        let pointQuery = 
        {
            query : `SELECT
                    ...
                    LEFT JOIN
                    CUSTOMERS AS CUST ON POINT.CUSTOMER = CUST.GUID
                    LEFT JOIN
        ...
        if (pointData.length)
        {
            for (let i = 0; i < pointData.length; i++) 
            {
                const cardInfo = 
                {
                    ...
                    "customerPoint": Math.trunc(Number(pointData[i].customerPoint))
                };
