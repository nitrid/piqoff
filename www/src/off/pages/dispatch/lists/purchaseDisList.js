...
export default class purchaseDisList extends React.PureComponent
{
    constructor(props)
    {
        ...
        [
            ...
            {CODE : "DOC_DATE",NAME : this.t("grdPurcDisList.clmDate")},
    ...
    async loadState()
    {
        let tmpLoad = await this.access.filter({ELEMENT:'grdPurcDisListState',USERS:this.user.CODE})
        return tmpLoad.getValue()
    }
    async saveState(e)
    {
        let tmpSave = await this.access.filter({ELEMENT:'grdPurcDisListState',USERS:this.user.CODE,PAGE:this.props.data.id,APP:"OFF"})
            ...
            source : 
            {
                ...
                select : 
                {
                    query : "SELECT * FROM DOC_VW_01 " +
                            ...
                            " AND TYPE = 0 AND DOC_TYPE = 40 AND REBATE = 0 ORDER BY DOC_DATE DESC,REF_NO DESC",
                    ...
                    <div className="row px-2 pt-2">
                        <div className="col-12">
                            ...
                            >                            
                                ...
                                <StateStoring enabled={true} type="custom" customLoad={this.loadState} customSave={this.saveState} storageKey={this.props.data.id + "_grdPurcDisList"}/>
                                ...
                                <Column dataField="DOC_DATE" caption={this.t("grdPurcDisList.clmDate")} visible={true} width={200} dataType="datetime" format={"dd/MM/yyyy"}/> 
                                <Column dataField="AMOUNT" caption={this.t("grdPurcDisList.clmAmount")} visible={false} format={{ style: "currency", currency: Number.money.code,precision: 2}}/> 
                                <Column dataField="VAT" caption={this.t("grdPurcDisList.clmVat")} visible={false} format={{ style: "currency", currency: Number.money.code,precision: 2}}/> 
                                <Column dataField="TOTAL" caption={this.t("grdPurcDisList.clmTotal")} visible={true} format={{ style: "currency", currency: Number.money.code,precision: 2}}/>              
                            </NdGrid>
                        </div>
                    </div>
                </ScrollView>
