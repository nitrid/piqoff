    ...
    async _btnGetClick()
    {
        ...
        let tmpSource =
        {
            source : 
            {
                ...
                select : 
                {
                    query : "SELECT DOC_GUID AS GUID,REF,REF_NO,INPUT_CODE,OUTPUT_CODE,OUTPUT_NAME,MAX(DOC_DATE) AS DOC_DATE_CONVERT,SUM(AMOUNT) AS AMOUNT,SUM(VAT) AS VAT, SUM(TOTAL) AS TOTAL " +
                            "WHERE ((INPUT_CODE = @INPUT_CODE) OR (@INPUT_CODE = '')) AND "+ 
                            "((DOC_DATE >= @FIRST_DATE) OR (@FIRST_DATE = '19700101')) AND ((DOC_DATE <= @LAST_DATE) OR (@LAST_DATE = '19700101'))  " +
                            " AND  TYPE = 1 AND DOC_TYPE = 40  AND REBATE = 0 AND INVOICE_DOC_GUID = '00000000-0000-0000-0000-000000000000'  GROUP BY REF,REF_NO,INPUT_CODE,OUTPUT_CODE,OUTPUT_NAME,DOC_GUID  ",
