...
export default class branchPurcDispatch extends DocBase
{
    ...
    saveState(e)
    {
        let tmpSave = this.access.filter({ELEMENT:'grdSlsDispatchState',USERS:this.user.CODE,PAGE:this.props.data.id,APP:'OFF',EMPTY:true})
        tmpSave.setValue(e)
        tmpSave.save()
    }
    async init()
                                ...
                                <Item location="after" locateInMenu="auto">
                                    ...
                                    onClick={async ()=>
                                    {
                                        ...
                                        else if(this.docObj.dt()[0].LOCKED == 2)
                                        {
                                            let tmpConfObj =
                                            {
                                                id:'msgLockedType2',showTitle:true,title:this.t("msgLockedType2.title"),showCloseButton:true,width:'500px',height:'auto',
                                ...
                                <NdItem>
                                    <React.Fragment>
                                        ...
                                        onRowRemoving={async (e)=>
                                        {
                                            ...
                                            {
                                                e.cancel = true
                                                this.toast.show({message:this.t("msgRowNotDelete.msg"),type:"warning"})
                                                e.component.cancelEditData()
                                            }
