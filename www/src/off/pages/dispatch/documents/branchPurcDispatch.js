...
export default class branchSaleDispatch extends React.PureComponent
{
    ...
    async getDoc(pGuid,pRef,pRefno)
    {
        ...
        await this.docObj.load({GUID:pGuid,REF:pRef,REF_NO:pRefno,TYPE:0,DOC_TYPE:42});
    ...
    _cellRoleRender(e)
    {
        ...
        if(e.column.dataField == "DISCOUNT")
        {
            return (
                ...
                onChange={(r)=>
                {
                    this.grdSlsDispatch.devGrid.cellValue(e.rowIndex,"DISCOUNT",r.component._changedValue)
                }}
                button=
                {
                    [
                ...
                if(this.combineControl == true)
                {
                    ...
                    await this.msgCombineItem.show().then(async (e) =>
                    {
                        if(e == 'btn01')
                        {
                            ...
                            this.docObj.docItems.dt()[i].TOTALHT =  parseFloat((this.docObj.docItems.dt()[i].AMOUNT - this.docObj.docItems.dt()[i].DISCOUNT).toFixed(4))
                            this._calculateTotal()                            
    ...
    async getDocs(pType)
    {
        ...
        if(pType == 0)
        {
            tmpQuery = 
            {
                query : "SELECT GUID,REF,REF_NO,OUTPUT_CODE,OUTPUT_NAME,DOC_DATE_CONVERT,TOTAL FROM DOC_VW_01 WHERE TYPE = 0 AND DOC_TYPE = 42 AND REBATE = 0 AND DOC_DATE > GETDATE()-30 ORDER BY DOC_DATE DESC"
            }
        }
        else
        {
            tmpQuery = 
            {
                query : "SELECT GUID,REF,REF_NO,OUTPUT_CODE,OUTPUT_NAME,DOC_DATE_CONVERT,TOTAL FROM DOC_VW_01 WHERE TYPE = 0 AND DOC_TYPE = 42 AND REBATE = 0 ORDER BY DOC_DATE DESC"
            }
        }
