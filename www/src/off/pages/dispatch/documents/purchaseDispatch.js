...
export default class purchaseDispatch extends DocBase
{
    ...
    async initPiqX()
    {
        ...
        if(jData.length > 0)
        {
            ...
            else
            {
                let tmpConfObj =
                {
                    id:'msgFourniseurNotFound',showTitle:true,title:this.t("msgFourniseurNotFound.title"),showCloseButton:true,width:'500px',height:'auto',
    ...
    saveState(e)
    {
        let tmpSave = this.access.filter({ELEMENT:'grdPurcDispatchState',USERS:this.user.CODE,PAGE:this.props.data.id,APP:"OFF"})
        tmpSave.setValue(e)
        tmpSave.save()
    }
    async init()
        ...
        this.pg_txtItemsCode.on('showing',()=>
        {
            this.pg_txtItemsCode.setSource(
            {
                source:
                {
                    select:
                    {
                        query : `SELECT GUID,CODE,NAME,VAT,COST_PRICE,UNIT,STATUS, ISNULL((SELECT TOP 1 CODE FROM ITEM_MULTICODE WHERE ITEM_MULTICODE.ITEM = ITEMS_VW_04.GUID AND ITEM_MULTICODE.CUSTOMER = '${this.docObj.dt()[0].OUTPUT}' AND DELETED = 0 ORDER BY LDATE DESC),'') AS MULTICODE, 
            ...
            <div id={this.props.data.id + this.props.data.tabkey}>
                <ScrollView>
                    ...
                    <div className="row px-2 pt-1" style={{height: '130px'}}>
                        <div className="col-12">
                            <NdForm colCount={3} id={"frmPurcDispatch"  + this.tabIndex}>
                                {/* txtRef-Refno */}
                                <NdItem>
                                ...
                                <NdItem>
                                    ...
                                    {{
                                        source:
                                        {
                                            select:
                                            {
                                                query : `SELECT GUID,CODE,TITLE,NAME,LAST_NAME,VAT_ZERO,[TYPE_NAME],[GENUS_NAME] FROM CUSTOMER_VW_03 WHERE (UPPER(CODE) LIKE UPPER(@VAL) OR UPPER(TITLE) LIKE UPPER(@VAL)) AND STATUS = 1`,
                                                param : ['VAL:string|50']
                                            },
                                            sql:this.core.sql
                                        }
                                ...
                                <NdItem>
                                    <React.Fragment>
                                        ...
                                        >
                                            ...
                                            <Column dataField="ITEM_CODE" caption={this.t("grdPurcDispatch.clmItemCode")} width={105} editCellRender={this.cellRoleRender}/>
                                            ...
                                            <Column dataField="ORIGIN" caption={this.t("grdPurcDispatch.clmOrigin")} width={60} allowEditing={true} editCellRender={this.cellRoleRender}/>
                                            <Column dataField="QUANTITY" caption={this.t("grdPurcDispatch.clmQuantity")} width={65} dataType={'number'} cellRender={(e)=>{return e.value + " / " + e.data.UNIT_SHORT}} editCellRender={this.cellRoleRender}/>
                                            ...
                                            <Column dataField="DISCOUNT" caption={this.t("grdPurcDispatch.clmDiscount")} width={60} editCellRender={this.cellRoleRender} dataType={'number'} format={{ style: "currency", currency: Number.money.code,precision: 2}}/>
                                            <Column dataField="DISCOUNT_RATE" caption={this.t("grdPurcDispatch.clmDiscountRate")} width={60} dataType={'number'} editCellRender={this.cellRoleRender}/>
