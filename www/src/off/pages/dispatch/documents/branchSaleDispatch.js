...
export default class branchSaleDispatch extends DocBase
{
    ...
    saveState(e)
    {
        let tmpSave = this.access.filter({ELEMENT:'grdSlsDispatchState',USERS:this.user.CODE,PAGE:this.props.data.id,APP:"OFF"})
        tmpSave.setValue(e)
        tmpSave.save()
    }
    async init()
    {
        ...
        this.pg_txtBarcode.on('showing',()=>
        {
            this.pg_txtBarcode.setSource(
            {
                ...
                {
                    select:
                    {   query : `SELECT ITEMS_VW_04.GUID,CODE,NAME,COST_PRICE,,ITEMS_VW_04.VAT,BARCODE,ITEMS_VW_04.UNIT,ISNULL((SELECT TOP 1 CODE FROM ITEM_MULTICODE WHERE ITEM_MULTICODE.ITEM = ITEMS_VW_04.GUID AND ITEM_MULTICODE.CUSTOMER = '${this.docObj.dt()[0].INPUT}' AND DELETED = 0 ORDER BY LDATE DESC),'') AS MULTICODE, 
                                ...
                                <Item location="after" locateInMenu="auto">
                                    ...
                                    onClick={async()=>
                                    {
                                        ...
                                        for (let i = 0; i < this.docObj.docItems.dt().length; i++) 
                                        {
                                            ...
                                            {
                                                this.toast.show({message:this.t("msgdocNotDelete.msg"),type:"warning"})
                                                return
                                            }
                                        }
                            ...
                            <NdForm colCount={1} onInitialized={(e)=>
                            {
                                ...
                                <React.Fragment>
                                    ...
                                    onRowUpdating={async (e)=>
                                    {
                                        ...
                                        {
                                            e.cancel = true
                                            this.toast.show({message:this.t("msgRowNotUpdate.msg"),type:"warning"})
                                    ...
                                    >
                                        ...
                                        <Column dataField="DISCOUNT" caption={this.t("grdSlsDispatch.clmDiscount")} width={60} editCellRender={this.cellRoleRender} dataType={'number'} format={{ style: "currency", currency: Number.money.code,precision: 3}}/>
                                        <Column dataField="DISCOUNT_RATE" caption={this.t("grdSlsDispatch.clmDiscountRate")} width={60} editCellRender={this.cellRoleRender} dataType={'number'} />
