...
import { genreCls} from '../../../../core/cls/items.js';
...
export default class genreCard extends React.PureComponent
{
    constructor(props)
    {
        super(props)
    ...
    async init()
    {
        this.genreObj.clearAll();
        this.genreObj.addEmpty();

        this.genreObj.ds.on('onAddRow',(pTblName,pData) =>
        {
            if(pData.stat == 'new')
            {
                if(this.prevCode != '')
        ...
        this.genreObj.ds.on('onEdit',(pTblName,pData) =>
        {            
        ...
        this.genreObj.ds.on('onRefresh',(pTblName) =>
        {            
            this.prevCode = this.genreObj.dt('ITEM_GENRE').length > 0 ? this.genreObj.dt('ITEM_GENRE')[0].CODE : '';
            this.btnBack.setState({disabled:true});
            this.btnNew.setState({disabled:false});
            this.btnSave.setState({disabled:true});
            this.btnDelete.setState({disabled:false});
        })
        this.genreObj.ds.on('onDelete',(pTblName) =>
        {            
            this.btnBack.setState({disabled:false});
            this.btnNew.setState({disabled:true});
            this.btnSave.setState({disabled:false});
    ...
    async getGenre(pCode)
    {
        this.genreObj.clearAll()
        await this.genreObj.load({CODE:pCode});
    }
    async checkSector(pCode)
    {
        return new Promise(async resolve =>
        {
            if(pCode !== '')
            {
                let tmpQuery = 
                {
                    query : `SELECT TOP 1 CODE FROM ITEM_GENRE_VW_01 WHERE CODE = @CODE`,
        ...
        return(
            <div>
                <ScrollView>
                    <div className="row px-2 pt-2">
                        <div className="col-12">
                            <Toolbar>
                                ...
                                <Item location="after" locateInMenu="auto">
                                    <NdButton id="btnSave" parent={this} icon="floppy" type="success" validationGroup={"frmGenre"  + this.tabIndex}
                                    onClick={async (e)=>
                                    {
                                ...
                                <Item location="after" locateInMenu="auto">
                                    ...
                                    onClick={async()=>
                                    {
                                        ...
                                        if(pResult == 'btn01')
                                        {
                                            this.genreObj.dt('ITEM_GENRE').removeAt(0)
                                            await this.genreObj.dt('ITEM_GENRE').delete();
                                            this.init(); 
                                        }
                    ...
                    <div className="row px-2 pt-2">
                        <div className="col-12">
                            <NdForm colCount={2} id="frmGenre">
                                 {/* txtCode */}
                                 <NdItem>
                                    <NdLabel text={this.t("txtCode")} alignment="right" />
                                    <NdTextBox id="txtCode" parent={this} simple={true} dt={{data:this.genreObj.dt('ITEM_GENRE'),field:"CODE"}}  
                                    ...
                                    >
                                        <Validator validationGroup={"frmGenre"  + this.tabIndex}>
                                ...
                                <NdItem>
                                    ...
                                    <NdTextBox id="txtTitle" parent={this} simple={true} dt={{data:this.genreObj.dt('ITEM_GENRE'),field:"NAME"}}
                                    upper={this.sysParam.filter({ID:'onlyBigChar',USERS:this.user.CODE}).getValue().value}
