...
export default class purchaseOrder extends DocBase
{
    ...
    saveState(e)
    {
        let tmpSave = this.access.filter({ELEMENT:'grdPurcOrdersState',USERS:this.user.CODE,PAGE:this.props.data.id,APP:"OFF"})
        tmpSave.setValue(e)
        tmpSave.save()
    }
    async init()
            ...
            this.pg_txtBarcode.setSource(
            {
                source:
                {
                    ...
                    {   query : `SELECT ITEMS_VW_04.GUID,CODE,NAME,COST_PRICE,VAT,BARCODE,ITEMS_VW_04.UNIT,ISNULL((SELECT TOP 1 CODE FROM ITEM_MULTICODE WHERE ITEM_MULTICODE.ITEM = ITEMS_VW_04.GUID AND ITEM_MULTICODE.CUSTOMER = '${this.docObj.dt()[0].OUTPUT}' AND DELETED = 0 ORDER BY LDATE DESC),'') AS MULTICODE, 
                                ...
                                FROM ITEMS_VW_04 INNER JOIN ITEM_BARCODE_VW_04 ON ITEMS_VW_04.GUID = ITEM_BARCODE_VW_04.ITEM_GUID WHERE  STATUS = 1 AND (ITEM_BARCODE_VW_04.BARCODE LIKE  '%' +@BARCODE)`,
            ...
            return (
                ...
                onChange={(async(r)=>
                {
                    if(typeof r.event.isTrusted == 'undefined')
                    {
                        let tmpQuery = 
                        {
                            query : `SELECT ITEMS_VW_04.GUID,CODE,NAME,ITEMS_VW_04.VAT,COST_PRICE,ITEMS_VW_04.UNIT FROM ITEMS_VW_04 INNER JOIN ITEM_BARCODE_VW_04 ON ITEMS_VW_04.GUID = ITEM_BARCODE_VW_04.ITEM_GUID WHERE CODE = @CODE OR ITEM_BARCODE_VW_04.BARCODE = @CODE`,
    ...
    addItem(pData,pIndex,pQuantity,pPrice)
    {
        return new Promise(async resolve => 
        {
            ...
            {
                pIndex = this.docObj.docOrders.dt().length - pIndex -1
            }
            ...
            if(typeof pPrice == 'undefined')
            {
               if(tmpCheckData.result.recordset.length > 0)
               {
                    this.docObj.docOrders.dt()[pIndex].PRICE = parseFloat((tmpCheckData.result.recordset[0].PRICE).toFixed(4))
                    this.docObj.docOrders.dt()[pIndex].DISCOUNT = Number(tmpCheckData.result.recordset[0].PRICE  * pQuantity).rateInc(tmpDiscRate,4)
                    this.docObj.docOrders.dt()[pIndex].DISCOUNT_RATE = tmpDiscRate
                    this.docObj.docOrders.dt()[pIndex].VAT = parseFloat((((tmpCheckData.result.recordset[0].PRICE  * pQuantity) - this.docObj.docOrders.dt()[pIndex].DISCOUNT) * (this.docObj.docOrders.dt()[pIndex].VAT_RATE / 100)).toFixed(6))
                    this.docObj.docOrders.dt()[pIndex].AMOUNT = parseFloat((tmpCheckData.result.recordset[0].PRICE * pQuantity).toFixed(4))
                    this.docObj.docOrders.dt()[pIndex].TOTAL = Number(((tmpCheckData.result.recordset[0].PRICE * pQuantity)+ this.docObj.docOrders.dt()[pIndex].VAT)).round(2)
                    ...
                    this.docObj.docOrders.dt()[pIndex].SUB_PRICE = Number(parseFloat((tmpCheckData.result.recordset[0].PRICE).toFixed(4)) / this.docObj.docOrders.dt()[pIndex].SUB_FACTOR).round(2)
            ...
            <div id={this.props.data.id + this.props.data.tabkey}>
                <ScrollView>
                                ...
                                <Item location="after" locateInMenu="auto">
                                    ...
                                    onClick={async (e)=>
                                    {
                                        ...
                                        if(typeof this.docObj.docOrders.dt()[0] == 'undefined')
                                        {
                                            let tmpConfObj =
                                            {
                                                id:'msgNotRow',showTitle:true,title:this.lang.t("msgNotRow.title"),showCloseButton:true,width:'500px',height:'auto',
                                            ...
                                            for (let i = 0; i < this.docObj.docOrders.dt().length; i++) 
                                            {
                                                let tmpQuery = 
                                                {
                                                    query : `SELECT ISNULL((SELECT NAME FROM ITEMS WHERE ITEMS.GUID = ITEM_MULTICODE.ITEM),'') AS ITEM_NAME,ISNULL((SELECT CODE FROM ITEMS WHERE ITEMS.GUID = ITEM_MULTICODE.ITEM),'') AS ITEM_CODDE, 
                                                            ISNULL((SELECT TITLE FROM CUSTOMER_VW_03 WHERE CUSTOMER_VW_03.GUID = ITEM_MULTICODE.CUSTOMER),'') AS CUSTOMER_NAME,CODE AS MULTICODE, 
                    ...
                    <div className="row px-2 pt-1" style={{height: '130px'}}>
                        <div className="col-12">
                            <NdForm colCount={3} id="frmPurcOrder">
                                {/* txtRef-Refno */}
                                <NdItem>
                                ...
                                <NdItem>
                                    <React.Fragment>
                                        ...
                                        >
                                            ...
                                            <Column dataField="ITEM_CODE" caption={this.t("grdPurcOrders.clmItemCode")} width={105} editCellRender={this.cellRoleRender}/>
                                            ...
                                            <Column dataField="QUANTITY" caption={this.t("grdPurcOrders.clmQuantity")} width={65} dataType={'number'} cellRender={(e)=>{return e.value + " / " + e.data.UNIT_SHORT}} editCellRender={this.cellRoleRender}/>
                                            <Column dataField="SUB_FACTOR" caption={this.t("grdPurcOrders.clmSubFactor")} width={65} allowEditing={true} cellRender={(e)=>{return e.value + " / " + e.data.SUB_SYMBOL}}/>
