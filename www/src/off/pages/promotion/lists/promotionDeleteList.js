...
export default class promotionDeleteList extends React.PureComponent
{
    constructor(props)
    {
        super(props)
    ...
    {        
        let tmpSource =
        {
            source : 
            {
                ...
                select : 
                {
                    query : "SELECT *,CASE APP_TYPE WHEN 0 THEN CONVERT(NVARCHAR,APP_AMOUNT) + ' %' WHEN 5 THEN CONVERT(NVARCHAR,APP_AMOUNT) + ' " + Number.money.sign + "' ELSE '' END AS APP_VALUE FROM PROMO_COND_APP_DELETE_VW_01 WHERE ((CODE LIKE '%' + @CODE + '%') OR (COND_ITEM_CODE LIKE '%' + @CODE + '%') OR " + 
