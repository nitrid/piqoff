...
export default class piqXPurcFactList extends React.PureComponent
{
    ...
    async btnGetClick()
    {
        if(this.taxId != "")
        {
            this.core.socket.emit('piqXInvoiceList',{taxId:this.taxId,first:moment(this.dtFirst.value).utcOffset(0, true),last:moment(this.dtLast.value).utcOffset(0, true),docType:40,rebate:0},async(pData)=>
            {
                                        ...
                                        async(pData) =>
                                        {
                                            if(typeof pData != 'undefined' && typeof pData.err == 'undefined' && pData.length > 0)
                                            {
                                                try
                                                {
                                                    ...
                                                    {
                                                        id: 'irs_02_001',
                                                        text: this.lang.t('menuOff.irs_02_001'),
