                                        ...
                                        <div className='col-6'>
                                            ...
                                            {       
                                                if(e.validationGroup.validate().status == "valid")
                                                {
                                                    // let tmpQuery = 
                                                    // {
                                                    //     query : "SELECT CUS.REF,CUS.REF_NO,CUS.OUTPUT_CODE,CUS.OUTPUT_NAME,CUS.DOC_DATE,DEPT.PAYING_AMOUNT AS AMOUNT, " +
                                                    //             "ISNULL((SELECT TOP 1 PATH FROM LABEL_DESIGN WHERE TAG = @DESIGN),'') AS PATH,   " +
                                                    //             "CASE WHEN CUS.PAY_TYPE = 0 THEN 'Nakit' WHEN CUS.PAY_TYPE = 1 THEN 'Cek' WHEN CUS.PAY_TYPE = 2 THEN 'Banka Havale' ELSE '' END AS PAY_TYPE, " +
                                                    //             "CUS.INPUT_NAME, " +
                                                    //             "DEPT.DOC_REF_NO  AS PAYING_DOC " +
                                                    //             "FROM DOC_CUSTOMER_VW_01 AS CUS INNER JOIN DEPT_CREDIT_MATCHING_VW_03 AS DEPT ON CUS.GUID = DEPT.PAYING_DOC " +
                                                    //             "WHERE ((CUS.OUTPUT_CODE = @OUTPUT_CODE) OR (@OUTPUT_CODE = '')) AND "+ 
                                                    //             "((CUS.DOC_DATE >= @FIRST_DATE) OR (@FIRST_DATE = '19700101')) AND ((CUS.DOC_DATE <= @LAST_DATE) OR (@LAST_DATE = '19700101'))  " +
                                                    //             " AND CUS.TYPE = 0 AND CUS.DOC_TYPE = 200 ORDER BY CUS.DOC_DATE DESC",
