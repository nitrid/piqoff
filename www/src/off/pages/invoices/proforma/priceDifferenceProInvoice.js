...
export default class priceDifferenceInvoice extends React.PureComponent
{
    ...
    async getDoc(pGuid,pRef,pRefno)
    {
        ...
        await this.docObj.load({GUID:pGuid,REF:pRef,REF_NO:pRefno,TYPE:1,DOC_TYPE:101});
                    ...
                    [
                        {
                            ...
                            onClick:async ()  =>
                            {
                                ...
                                {
                                    this.grdDiffInv.devGrid.cellValue(e.rowIndex,"QUANTITY",this.txtTotalQuantity.value)
                                    this.grdDiffInv.devGrid.cellValue(e.rowIndex,"UNIT",this.cmbUnit.value)
                                    this.grdDiffInv.devGrid.cellValue(e.rowIndex,"UNIT_FACTOR",this.txtUnitFactor.value )
                                });  
                            }
                        },
                    ]
    ...
    async _getContract()
    {
        ...
        this.pg_contractGrid.onClick = async(data) =>
        {
            ...
            for (let i = 0; i < data.length; i++) 
            {
                let tmpQuery = 
                {
                    query :"SELECT * FROM DOC_ITEMS_VW_01 WHERE (((DOC_GUID = @DOC_GUID) OR (INVOICE_GUID = @DOC_GUID)) OR (@DOC_GUID = '00000000-0000-0000-0000-000000000000')) AND ISNULL((SELECT TOP 1 INVOICE_GUID FROM DOC_ITEMS_VW_01 AS DIF WHERE DIF.INVOICE_GUID = DOC_ITEMS_VW_01.GUID AND DIF.DOC_TYPE =101),'00000000-0000-0000-0000-000000000000') = '00000000-0000-0000-0000-000000000000' ",
    ...
    async getDocs(pType)
    {
        ...
        if(pType == 0)
        {
            tmpQuery = 
            {
                query : "SELECT GUID,REF,REF_NO,INPUT_CODE,INPUT_NAME,DOC_DATE_CONVERT FROM DOC_VW_01 WHERE TYPE = 1 AND DOC_TYPE = 101 AND REBATE = 0 AND DOC_DATE > GETDATE()-30 ORDER BY DOC_DATE DESC"
            }
        }
        else
        {
            tmpQuery = 
            {
                query : "SELECT GUID,REF,REF_NO,INPUT_CODE,INPUT_NAME,DOC_DATE_CONVERT FROM DOC_VW_01 WHERE TYPE = 1 AND DOC_TYPE = 101 AND REBATE = 0 ORDER BY DOC_DATE DESC"
            }
        }
                                    ...
                                    <div className="row">
                                        <div className="col-4 pe-0">
                                            ...
                                            onValueChanged={(async()=>
                                            {
                                                let tmpQuery = 
                                                {
                                                    query :"SELECT ISNULL(MAX(REF_NO) + 1,1) AS REF_NO FROM DOC WHERE TYPE = 1 AND DOC_TYPE = 101 AND REF = @REF ",
                                                    param : ['REF:string|25'],
