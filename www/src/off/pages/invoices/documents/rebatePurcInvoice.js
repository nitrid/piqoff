...
import { deptCreditMatchingCls} from '../../../../core/cls/doc.js';
...
export default class rebatePurcInvoice extends DocBase
{
    constructor(props)
    {
        super(props)
        ...
        this.pg_txtBarcode.on('showing',()=>
            {
                this.pg_txtBarcode.setSource(
                {
                    source:
                    {
                        select:
                        {   
                            query : `SELECT ITEMS_VW_04.GUID,CODE,NAME,COST_PRICE,ITEMS_VW_04.UNIT,ITEMS_VW_04.VAT,BARCODE,PARTILOT_GUID,LOT_CODE, 
    ...
    async getRebate()
    {
        let tmpQuery = 
        {
            query : `SELECT *,REF + '-' + CONVERT(VARCHAR,REF_NO) AS REFERANS, 
                    ...
                    FROM DOC_ITEMS_VW_01 WHERE INPUT = @INPUT AND REBATE_DOC_GUID = '00000000-0000-0000-0000-000000000000' AND TYPE = 1 AND (DOC_TYPE IN(20) OR (DOC_TYPE = 40 AND INVOICE_DOC_GUID <> '00000000-0000-0000-0000-000000000000')) AND REBATE = 0`,
        ...
        for (let i = 0; i < this.tagItemCode.value.length; i++) 
        {
            if(this.cmbMultiItemType.value == 0)
            {
                let tmpQuery = 
                {
                    ...
                    ISNULL((SELECT TOP 1 CODE AS MULTICODE FROM ITEM_MULTICODE WHERE ITEM = ITEMS_VW_04.GUID AND CUSTOMER = '${this.docObj.dt()[0].OUTPUT}' AND DELETED = 0 ORDER BY LDATE DESC),'') AS MULTICODE
                    FROM ITEMS_VW_04 WHERE ISNULL((SELECT TOP 1 CODE AS MULTICODE FROM ITEM_MULTICODE WHERE ITEM = ITEMS_VW_04.GUID AND CUSTOMER = '${this.docObj.dt()[0].OUTPUT}' AND DELETED = 0 ORDER BY LDATE DESC),'') = @VALUE `,
                                            ...
                                            onClick:async ()  =>
                                            {
                                                let tmpQuery = 
                                                {
                                                    query : `SELECT NAME,ROUND(SUM(UNIT_FACTOR * QUANTITY),2) AS UNIT_FACTOR 
                                                            FROM ( SELECT ITEM_CODE,QUANTITY, (SELECT TOP 1 NAME FROM ITEM_UNIT_VW_01 WHERE ITEM_GUID= ITEM AND TYPE = 1 ) AS NAME, (SELECT TOP 1 FACTOR FROM ITEM_UNIT_VW_01 WHERE ITEM_GUID= ITEM AND TYPE = 1 ) AS UNIT_FACTOR 
