...
export default class salesInvoice extends React.PureComponent
{
    ...
    async _getOffers()
    {
        ...
        else
        {
            let tmpQuery = 
            {
                query : "SELECT *,REF + '-' + CONVERT(VARCHAR,REF_NO) AS REFERANS FROM DOC_OFFERS_VW_01 WHERE INPUT = @INPUT AND SHIPMENT_GUID = '00000000-0000-0000-0000-000000000000' AND TYPE = 1 AND DOC_TYPE IN(61)",
                param : ['INPUT:string|50'],
                value : [this.docObj.dt()[0].INPUT]
            }
            let tmpData = await this.core.sql.execute(tmpQuery) 
    ...
    async _getProforma()
    {
        ...
        else
        {
            let tmpQuery = 
            {
                query : "SELECT *,REF + '-' + CONVERT(VARCHAR,REF_NO) AS REFERANS FROM DOC_ITEMS_VW_01 WHERE INPUT = @INPUT AND INVOICE_GUID = '00000000-0000-0000-0000-000000000000' AND TYPE = 1 AND DOC_TYPE IN(120) AND REBATE = 0",
                param : ['INPUT:string|50'],
                value : [this.docObj.dt()[0].INPUT]
            }
