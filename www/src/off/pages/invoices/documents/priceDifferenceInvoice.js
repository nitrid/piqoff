...
export default class priceDifferenceInvoice extends React.PureComponent
{
    ...
    async addItem(pData,pIndex,pQuantity)
    {
                ...
                if(this.combineControl == true)
                {
                    ...
                    await this.msgCombineItem.show().then(async (e) =>
                    {
                        if(e == 'btn01')
                        {
                            ...
                            this._calculateTotal()
                            await this.grdDiffInv.devGrid.deleteRow(0)
        ...
        if(tmpData.result.recordset.length > 0)
        {
            ...
            this.docObj.docItems.dt()[pIndex].VAT = parseFloat((tmpData.result.recordset[0].PRICE * (pData.VAT / 100)).toFixed(4))
            this.docObj.docItems.dt()[pIndex].AMOUNT = parseFloat((tmpData.result.recordset[0].PRICE).toFixed(4))
            this.docObj.docItems.dt()[pIndex].TOTAL = parseFloat((tmpData.result.recordset[0].PRICE + this.docObj.docItems.dt()[pIndex].VAT).toFixed(4))
    ...
    async _getContract()
    {
        ...
        this.pg_contractGrid.onClick = async(data) =>
        {
            ...
            for (let i = 0; i < data.length; i++) 
            {
                let tmpQuery = 
                {
                    query :"SELECT * FROM DOC_ITEMS_VW_01 WHERE (((DOC_GUID = @DOC_GUID) OR (INVOICE_DOC_GUID = @DOC_GUID)) OR (@DOC_GUID = '00000000-0000-0000-0000-000000000000')) AND ISNULL((SELECT TOP 1 INVOICE_DOC_GUID FROM DOC_ITEMS_VW_01 AS DIF WHERE DIF.INVOICE_DOC_GUID = DOC_ITEMS_VW_01.GUID AND DIF.DOC_TYPE =21),'00000000-0000-0000-0000-000000000000') = '00000000-0000-0000-0000-000000000000' ",
    ...
    async getDocs(pType)
    {
        ...
        if(pType == 0)
        {
            tmpQuery = 
            {
                query : "SELECT GUID,REF,REF_NO,INPUT_CODE,INPUT_NAME,DOC_DATE_CONVERT FROM DOC_VW_01 WHERE TYPE = 1 AND DOC_TYPE = 21 AND REBATE = 0 AND DOC_DATE > GETDATE()-30 ORDER BY DOC_DATE DESC"
            }
        }
        else
        {
            tmpQuery = 
            {
                query : "SELECT GUID,REF,REF_NO,INPUT_CODE,INPUT_NAME,DOC_DATE_CONVERT FROM DOC_VW_01 WHERE TYPE = 1 AND DOC_TYPE = 21 AND REBATE = 0 ORDER BY DOC_DATE DESC"
            }
        }
                        ...
                        >
                            <Form colCount={1} height={'fit-content'}>
                                <Item>
                                    ...
                                    >
                                        <Validator validationGroup={"frmPriceDiffdes" + this.tabIndex}>
                                ...
                                <Item>
                                    <div className='row'>
                                        <div className='col-6'>
                                            <NdButton text={this.lang.t("btnPrint")} type="normal" stylingMode="contained" width={'100%'} validationGroup={"frmPriceDiffdes" + this.tabIndex}
                                            onClick={async (e)=>
                                            {       
                                                if(e.validationGroup.validate().status == "valid")
                                                {
                                    ...
                                    <div className='row py-2'>
                                        <div className='col-6'>
                                            <NdButton text={this.t("btnView")} type="normal" stylingMode="contained" width={'100%'} validationGroup={"frmPriceDiffInv" + this.tabIndex}
                                            onClick={async (e)=>
                                            {       
                                                if(e.validationGroup.validate().status == "valid")
                                                {
                                        ...
                                        <div className='col-6'>
                                            <NdButton text={this.t("btnMailsend")} type="normal" stylingMode="contained" width={'100%'}  validationGroup={"frmPriceDiffInv" + this.tabIndex}
                                            onClick={async (e)=>
                                            {    
                                                if(e.validationGroup.validate().status == "valid")
                                                {
