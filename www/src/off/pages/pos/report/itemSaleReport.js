    ...
    async _btnGetClick()
    {
        ...
        let tmpSource =
        {
            source : 
            {
                ...
                select : 
                {
                    query : "SELECT MIN(DOC_DATE) AS FIRST_DATE, MAX(DOC_DATE) AS LAST_DATE, SUM(QUANTITY) AS QUANTITY, (PRICE -(DISCOUNT/QUANTITY)) AS PRICE,(PRICE * SUM(QUANTITY)) AS AMOUNT,SUM(DISCOUNT) AS DISCOUNT,SUM(LOYALTY) AS LOYALTY,SUM(TOTAL) AS TOTAL,ITEM_NAME " +
                    "FROM POS_SALE_VW_01 WHERE (DOC_DATE >= @FIRST_DATE AND DOC_DATE <= @LAST_DATE) AND ((ITEM_GUID = @ITEM) OR (@ITEM = '00000000-0000-0000-0000-000000000000')) GROUP BY PRICE,ITEM_GUID,ITEM_NAME,(PRICE -(DISCOUNT/QUANTITY)) ORDER BY ITEM_NAME,MIN(DOC_DATE) " ,
