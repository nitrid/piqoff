...
export default class posSalesReport extends React.PureComponent
{
    ...
    async printReport()
    {
        let tmpQuery = 
        {
            ...
            "MAX(PATH) AS PATH,   " +
            "(ISNULL((SELECT SUM(AMOUNT - CHANGE) FROM POS_PAYMENT_VW_01 WHERE POS_PAYMENT_VW_01.DOC_DATE =  TMP.DOC_DATE AND STATUS = 1 AND PAY_TYPE =0 AND TYPE = 0),0)- ISNULL((SELECT SUM(AMOUNT - CHANGE) FROM POS_PAYMENT_VW_01 WHERE POS_PAYMENT_VW_01.DOC_DATE =  TMP.DOC_DATE AND STATUS = 1 AND PAY_TYPE =0 AND TYPE = 1),0)) AS ESC,     " +
            "(ISNULL((SELECT SUM(AMOUNT - CHANGE) FROM POS_PAYMENT_VW_01 WHERE POS_PAYMENT_VW_01.DOC_DATE =  TMP.DOC_DATE AND STATUS = 1 AND PAY_TYPE =1 AND TYPE = 0),0) -ISNULL((SELECT SUM(AMOUNT - CHANGE) FROM POS_PAYMENT_VW_01 WHERE POS_PAYMENT_VW_01.DOC_DATE =  TMP.DOC_DATE AND STATUS = 1 AND PAY_TYPE =1 AND TYPE = 1),0)) AS CB,   " +
            "(ISNULL((SELECT SUM(AMOUNT - CHANGE) FROM POS_PAYMENT_VW_01 WHERE POS_PAYMENT_VW_01.DOC_DATE =  TMP.DOC_DATE AND STATUS = 1 AND PAY_TYPE =3 AND TYPE = 0),0) - ISNULL((SELECT SUM(AMOUNT - CHANGE) FROM POS_PAYMENT_VW_01 WHERE POS_PAYMENT_VW_01.DOC_DATE =  TMP.DOC_DATE AND STATUS = 1 AND PAY_TYPE =3 AND TYPE = 1),0)) AS TICKET,   " +
            "(ISNULL((SELECT SUM(AMOUNT - CHANGE) FROM POS_PAYMENT_VW_01 WHERE POS_PAYMENT_VW_01.DOC_DATE =  TMP.DOC_DATE AND STATUS = 1 AND PAY_TYPE NOT IN(0,1,3) AND TYPE = 0),0) -ISNULL((SELECT SUM(AMOUNT - CHANGE) FROM POS_PAYMENT_VW_01 WHERE POS_PAYMENT_VW_01.DOC_DATE =  TMP.DOC_DATE AND STATUS = 1 AND PAY_TYPE NOT IN(0,1,3) AND TYPE = 1),0)) AS OTHER   " +
            "    FROM  " +
            "(SELECT DOC_DATE,POS_GUID, " +
            "CASE WHEN TYPE  = 0 THEN FAMOUNT WHEN TYPE  = 1 THEN (FAMOUNT * -1) END  AS FAMOUNT,   " +
            "CASE WHEN TYPE  = 0 THEN VAT WHEN TYPE  = 1 THEN (VAT * -1) END  AS VAT,   " +
            "CASE WHEN TYPE  = 0 THEN TOTAL WHEN TYPE  = 1 THEN (TOTAL * -1) END  AS TOTAL,   " +
            "ISNULL((SELECT TOP 1 PATH FROM LABEL_DESIGN WHERE TAG = 72),'') AS PATH   " +
            "FROM POS_SALE_VW_01 WHERE  STATUS = 1) AS TMP   " +
