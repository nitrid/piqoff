...
export default class discountReport extends React.PureComponent
{
    constructor(props)
    {
        super(props)
        ...
        this.state = 
        {
            columnListValue : ['DOC_DATE','DISCOUNT']
        }
    ...
    async _btnGetirClick()
    {
        ...
        let tmpQuery = {
            query : "SELECT COUNT(*) AS TOTAL FROM POS WHERE DISCOUNT > 0 AND DOC_DATE >= @FIRST_DATE AND DOC_DATE <= @LAST_DATE",
        ...
        let tmpSource =
        {
            source : 
            {
                ...
                select : 
                {
                    query : "SELECT DOC_DATE,SUM(DISCOUNT) AS DISCOUNT FROM POS WHERE DISCOUNT > 0 AND DOC_DATE >= @FIRST_DATE AND DOC_DATE <= @LAST_DATE GROUP BY DOC_DATE ORDER BY DOC_DATE",
                    param : ['FIRST_DATE:date','LAST_DATE:date'],
                    value : [this.dtDate.startDate,this.dtDate.endDate]
                },
                sql : this.core.sql
