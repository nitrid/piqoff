            ...
            source : 
            {
                select : 
                {
                    query : `SELECT ITEM_CODE,MAX(ITEM_NAME) AS ITEM_NAME,SUM(QUANTITY) AS QUANTITY,ROUND(SUM(TOTALHT),2) AS AMOUNT,ROUND(SUM(VAT),2) AS VAT,ROUND(SUM(TOTAL),2) AS TOTAL 
                            FROM DOC_ITEMS_VW_01 WHERE DOC_DATE >= @FIRST_DATE AND DOC_DATE <= @LAST_DATE AND REBATE = 0 AND TYPE = 1 AND (DOC_TYPE =20 OR (DOC_TYPE = 40 AND INVOICE_DOC_GUID <> '00000000-0000-0000-0000-000000000000')) 
                            GROUP BY  ITEM_CODE`,
