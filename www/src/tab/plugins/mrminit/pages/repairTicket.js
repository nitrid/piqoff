...
export default class repairTicket extends React.PureComponent
{
    ...
    init()
    {
        this.socketEx = io('http://192.168.1.12:1453',{timeout:100000,transports : ['websocket']});

        this.socketEx.on('connect', () => 
        {
            console.log('Sunucuya bağlandı');
        });
        this.socketEx.on('connect_error', (error) => 
        {
            console.log(error)
        });
        this.socketEx.on('error', (error) => 
        {
            console.log(error)
        });
    }
    ...
    {
        this.dtList.clear()
        let tmpData = await this.execute({type:'select',data:{REF:''}})
        this.dtList.import(tmpData.result.recordset)
    ...
    execute(pData)
    {
        ...
        {
            this.socketEx.emit('execute', pData,(e)=>
            {
                resolve(e)
            })
        });
                ...
                <div style={{paddingLeft:"10px",paddingRight:"10px",paddingTop:"65px"}}>
                    ...
                    <div className='row pt-2' style={{height:'95%'}}>
                        <div className='col-12'>
                            ...
                            {
                                this.dtTicket.clear()
                                let tmpData = await this.execute({type:'select',data:{REF:e.data.REF}})
                                this.dtTicket.import(tmpData.result.recordset)
                                this.popRepairTicket.show()
                            }}
                            >
                ...
                <div>
                    <NbPopUp id={"popRepairTicket"} parent={this} title={""} fullscreen={true} onShowed={()=>
                    {
                        ...
                        <div>
                            <div className='row' style={{paddingTop:"10px"}}>
                                <div className='col-1 offset-10' align={"right"}>
                                    ...
                                    onClick={async()=>
                                    {
                                        ...
                                        if(this.dtTicket[0].stat == 'new')
                                        {
                                            await this.execute({type:'insert',data:this.dtTicket[0]})  
                                        }
                                        else if(this.dtTicket[0].stat == 'edit')
                                        {
                                            await this.execute({type:'update',data:this.dtTicket[0]})  
                                        }
                            ...
                            <div className='row' style={{paddingTop:"10px"}}>
                                <div className='col-6'>
                                    <NdSelectBox simple={true} parent={this} id="cmbCategory" displayExpr={'NAME'} valueExpr={'ID'} value={"VILLE_PLATE_BASSE"}
                                    title={this.t("cmbCategory")} dt={{data:this.dtTicket,field:"CATEGORY"}}
                                    data={{source : 
                                    [
                                ...
                                <div className='col-6'>
                                    <NdSelectBox simple={true} parent={this} id="cmbRepair" displayExpr={'NAME'} valueExpr={'ID'} value={"FERMETURE_ECLAIR_INF_20_CM"}
                                    title={this.t("cmbRepair")} dt={{data:this.dtTicket,field:"REPAIRS"}}
                                    data={{source : 
                                    [
