...
export default class rebateDispatch extends React.PureComponent
{
    ...
    constructor(props)
    {
        ...
        {
            query : `SELECT * FROM ITEMS_BARCODE_MULTICODE_VW_01 WHERE (CODE = @CODE OR BARCODE = @CODE OR (MULTICODE = @CODE AND CUSTOMER_GUID = @CUSTOMER_GUID)) OR (@CODE = '') ORDER BY LOT_CODE ASC`,
    ...
    getItem(pCode)
    {
        return new Promise(async resolve => 
        {
            ...
            if(this.itemDt.length > 0)
            {
                for (let i = 0; i < this.itemDt.length; i++) 
                {
                    ...
                    if(i + 1 == this.itemDt.length)
                    {
                        let tmpConfObj = 
                        {
                            id:'msgDiffCustomer',showTitle:true,title:this.lang.t("msgCombineItem.title"),showCloseButton:true,width:'400px',height:'auto',
                            ...
                            content: this.createStyledDialog('warning', this.t("msgDiffCustomer.title"), this.lang.t("msgDiffCustomer.msg"), 'fa-solid fa-user-times')
                        }
            ...
            {                               
                ...
                this.alertContent.content = this.createStyledDialog('error', 'Barkod Hatası', this.t("msgAlert.msgBarcodeNotFound"), 'fa-solid fa-barcode')
    ...
    async calcEntry() 
    {
        if (this.txtFactor.value !== 0 || this.txtQuantity.value !== 0 || this.txtPrice.value !== 0) 
        {
            ...
            this.txtEntrySumAmount.value = Number(Number(this.txtEntryAmount.value) + Number(this.txtEntryVat.value)).round(2);
        }
    }
    async addItem()
    {
        if(this.itemDt.length == 0)
        {
            this.alertContent.content = this.createStyledDialog('warning', 'Barkod Kontrolü', this.t("msgAlert.msgBarcodeCheck"), 'fa-solid fa-exclamation-triangle')
            await dialog(this.alertContent);
            return
        }
        ...
        {
            this.alertContent.content = this.createStyledDialog('warning', 'Miktar Kontrolü', this.t("msgAlert.msgQuantityCheck"), 'fa-solid fa-calculator')
            await dialog(this.alertContent);
            return
        }
                ...
                if(this.docObj.docItems.dt()[i].ITEM_CODE == this.itemDt[0].CODE)
                {
                    if(prmRowMerge == 2)
                    {
                        ...
                        let tmpConfObj = 
                        {
                            ...
                            content: this.createStyledDialog('info', 'Ürün Birleştirme', this.lang.t("msgCombineItem.msg"), 'fa-solid fa-layer-group')
                        }
    ...
    async onClickBarcodeShortcut()
    {
        ...
        {
            this.alertContent.content = this.createStyledDialog('warning', 'Depo Seçimi', this.t("msgAlert.msgDepot"), 'fa-solid fa-warehouse')
            await dialog(this.alertContent);
            return
        }
        if(this.docObj.dt()[0].INPUT == '')
        {
            this.alertContent.content = this.createStyledDialog('warning', 'Müşteri Seçimi', this.t("msgAlert.msgCustomer"), 'fa-solid fa-user')
            await dialog(this.alertContent);
            return
        }
    ...
    async onClickProcessShortcut()
    {
        ...
        {
            this.alertContent.content = this.createStyledDialog('warning', 'İşlem Kontrolü', this.t("msgAlert.msgProcess"), 'fa-solid fa-file-lines')
            await dialog(this.alertContent);
            return
        }
    ...
    render()
    {
        return(
            <div>
                <div>
                    <PageBar id={"pageBar"} parent={this} title={this.lang.t("menu.irs_04")} content=
                    {[
                        {
                    ...
                    <PageView id={"pageView"} parent={this} onActivePage={(e)=>{this.pageBar.activePage(e)}}>
                                                                ...
                                                                try 
                                                                {
                                                                    let tmpQuery = 
                                                                    {
                                                                        query : `SELECT ISNULL(MAX(REF_NO) + 1,1) AS REF_NO FROM DOC 
                                                                                WHERE TYPE = 1 AND DOC_TYPE = 40 AND REF = @REF AND REBATE = 1`,
                                                                        param : ['REF:string|25'],
                                                                ...
                                                                source:
                                                                {
                                                                    select:
                                                                    {
                                                                        query : `SELECT GUID,REF,REF_NO,INPUT_CODE,INPUT_NAME,DOC_DATE_CONVERT 
                                                                                FROM DOC_VW_01 WHERE TYPE = 1 AND DOC_TYPE = 40 AND REBATE = 1 
                            ...
                            <div className='row px-2'>
                                <div className='col-12'>
                                    ...
                                    <div className='card mb-2' style={{background: '#ffffff',borderRadius: '8px',boxShadow: '0 2px 8px rgba(0,0,0,0.06)',border: '1px solid #e9ecef',padding: '6px'}}>
                                        ...
                                        {/* Hesaplanan Değerler */}
                                        <div style={{background: '#f8f9fa',padding: '8px',borderRadius: '6px',border: '1px solid #dee2e6'}}>
                        ...
                        <PageContent id={"Process"}>
                            <div className='row px-2'>
                                <div className='col-12'>
                                    {/* Grid Container */}
                                    <div className='card mb-2' style={{background: '#ffffff',borderRadius: '8px',boxShadow: '0 2px 8px rgba(0,0,0,0.06)',border: '1px solid #e9ecef',padding: '4px'}}>
                                    ...
                                    <div className='card' style={{background: '#ffffff',borderRadius: '8px',boxShadow: '0 2px 8px rgba(0,0,0,0.06)',border: '1px solid #e9ecef',padding: '2px'}}>
                                        <div className='summary-container' style={{display: 'flex',flexDirection: 'column',gap: '2px'}}>
                                            ...
                                            <div className='summary-row' style={{display: 'flex',justifyContent: 'space-between',alignItems: 'center',padding: '4px 6px',background: 'linear-gradient(135deg, #28a745 0%, #20c997 100%)',borderRadius: '6px',border: '2px solid #28a745',boxShadow: '0 2px 8px rgba(40,167,69,0.2)'}}>
                                                ...
                                                <div style={{width: '120px'}}>
                                                    ...
                                                    style={{borderRadius: '4px',border: '2px solid #ffffff',fontSize: '14px',padding: '1px',backgroundColor: '#ffffff',textAlign: 'right',fontWeight: 'bold',color: '#28a745'}}/>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
