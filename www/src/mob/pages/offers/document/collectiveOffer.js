...
import { docCls,docOrdersCls,docOffers, docCustomerCls } from '../../../../core/cls/doc.js';
...
export default class collectiveOffer extends React.Component
{
    constructor()
    {
        super()
    ...
    async init()
    {
        ...
        await this.grdpurcOffer.dataRefresh({source:this.docObj.docOffers.dt('DOC_OFFERS')});
    }
            ...
            <div>
                 <div className="row px-2 pt-2">
                    <div className="row px-2 pt-2" style={{visibility:this.state.tbMain,position:"absolute"}}>
                        <Form colCount={1}>
                            ...
                            <Item>
                                ...
                                data={{source:{select:{query : "SELECT GUID,REF,REF_NO,OUTPUT_CODE,OUTPUT_NAME FROM DOC_VW_01 WHERE TYPE = 0 AND DOC_TYPE = 61 AND REBATE = 0"},sql:this.core.sql}}}
                                button=
                                {
                                    [
                                        {
                    ...
                    <div className="row px-2 pt-2" style={{visibility:this.state.tbDocument,position:"absolute"}}>
                        ...
                        <Item>
                            <NdGrid parent={this} id={"grdpurcOffer"} 
                            showBorders={true} 
                            ...
                            onRowUpdated={async(e)=>{
                                ...
                                {
                                    e.key.DISCOUNT = parseFloat((((this.docObj.docOffers.dt()[rowIndex].AMOUNT * e.data.DISCOUNT_RATE) / 100)).toFixed(2))
                                }
                                ...
                                {
                                    this.docObj.docOffers.dt()[rowIndex].DISCOUNT_RATE = parseFloat((100 - ((((e.key.PRICE * e.key.QUANTITY) - e.key.DISCOUNT) / (e.key.PRICE * e.key.QUANTITY)) * 100)).toFixed(2))
                                }
                            ...
                            >
                                ...
                                <Column dataField="ITEM_NAME" caption={this.t("grdpurcOffer.clmItemName")} width={350} />
                                <Column dataField="QUANTITY" caption={this.t("grdpurcOffer.clmQuantity")} dataType={'number'} width={80}/>
                                <Column dataField="PRICE" caption={this.t("grdpurcOffer.clmPrice")} dataType={'number'} format={{ style: "currency", currency: "EUR",precision: 3}} width={80}/>
                                <Column dataField="AMOUNT" caption={this.t("grdpurcOffer.clmAmount")} allowEditing={false} format={{ style: "currency", currency: "EUR",precision: 3}} width={80}/>
                                <Column dataField="DISCOUNT" caption={this.t("grdpurcOffer.clmDiscount")} dataType={'number'} format={{ style: "currency", currency: "EUR",precision: 3}} width={80}/>
                                <Column dataField="DISCOUNT_RATE" caption={this.t("grdpurcOffer.clmDiscountRate")} dataType={'number'} width={80}/>
                                <Column dataField="VAT" caption={this.t("grdpurcOffer.clmVat")} format={{ style: "currency", currency: "EUR",precision: 3}} allowEditing={false} width={80}/>
                                <Column dataField="TOTAL" caption={this.t("grdpurcOffer.clmTotal")} format={{ style: "currency", currency: "EUR",precision: 3}} allowEditing={false} width={100}/>
                            </NdGrid>
                        </Item>
                        </Form>
